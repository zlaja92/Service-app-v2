...
// `intervention_fault_leak` is mapped so report tests can assert the fault
// description is actually translated (not passed through as the raw key).
const translocoLangs = { en: { intervention_fault_leak: 'Curi voda' } };
...
describe('InterventionDetailPage', () => {
  ...
  function setup(
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: InterventionReportService, useValue: mockInterventionReportService },
      ...
      it(`EXP2-IDP-DEVTYPE: ${label} → component loads without error`, async () => {
        ...
        await TestBed.configureTestingModule({
          ...
          providers: [
            ...
            { provide: InterventionReportService, useValue: createMockInterventionReportService() },
  ...
  // =========================================================================
  // onReport() — delegates to InterventionReportService
  // (report context building + rendering is covered in intervention-report.service.spec)
  // =========================================================================

  describe('onReport() — delegates to InterventionReportService', () => {
    it('device is null \u2192 interventionReportService.open NOT called', async () => {
      mockInterventionService.getInterventionById.and.resolveTo({ interventionType: 'repair' });
      setup({ sn: 'SN001', id: 'int-report' }, null, true);
      await component.ionViewWillEnter();
      await component.onReport();
      expect(mockInterventionReportService.open).not.toHaveBeenCalled();
    });

    it('registration path (no intervention data) \u2192 open NOT called', async () => {
    ...
    it('happy path \u2192 wraps in a loader and delegates open(sn, device, data)', async () => {
      mockInterventionService.getInterventionById.and.resolveTo({ interventionType: 'repair', note: 'x' });
      const device = createMockDevice();
      setup({ sn: 'SN001', id: 'int-report' }, device);
      await component.ionViewWillEnter();
      await component.onReport();
      expect(mockLoadingAlert.wrap).toHaveBeenCalled();
      expect(mockInterventionReportService.open).toHaveBeenCalledWith(
        'SN001', device, jasmine.objectContaining({ interventionType: 'repair' }),
      );
    });
  });

  // =========================================================================
  // canReport — Report button visibility gated by the pdfReports feature flag
  // =========================================================================

  describe('canReport — gated by pdfReports feature flag', () => {
    function enablePdfReports(enabled: boolean): void {
      const cfg = getDefaultConfig();
      cfg.features.pdfReports = enabled;
      mockConfigStore.setConfig(cfg);
    }

    it('intervention + pdfReports enabled → canReport is true', async () => {
      enablePdfReports(true);
      mockInterventionService.getInterventionById.and.resolveTo({ interventionType: 'repair' });
      setup({ sn: 'SN001', id: 'int-1' }, createMockDevice());
      await component.ionViewWillEnter();
      expect((component as unknown as { canReport: boolean }).canReport).toBeTrue();
    });

    it('intervention + pdfReports disabled → canReport is false', async () => {
      enablePdfReports(false);
      mockInterventionService.getInterventionById.and.resolveTo({ interventionType: 'repair' });
      setup({ sn: 'SN001', id: 'int-1' }, createMockDevice());
      await component.ionViewWillEnter();
      expect((component as unknown as { canReport: boolean }).canReport).toBeFalse();
    });

    it('registration → canReport is false even when pdfReports enabled', async () => {
      enablePdfReports(true);
      mockInterventionService.getRegistration.and.resolveTo({ registeredAt: '2024-01-01' });
      setup({ sn: 'SN001', id: 'registration' });
      await component.ionViewWillEnter();
